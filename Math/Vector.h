...
struct mat4 {
	...
	mat4(const mat3& mat, const vec4& translation)
	{
		for (int i = 0; i < 3; ++i)
		{
			rows[i] = vec4(mat.rows[i], 0);
		}
		rows[3] = translation;
	}
...
mat3 lookAtRotation(const vec3& eye, const vec3& at, const vec3& up);
mat4 lookAtMatrix(const vec3& eye, const vec3& at, const vec3& up);
