...
public:
	...
	const MaterialLibResId& getMaterialLib() const { return materialLibId; }
	const std::string& getBatchMaterialName(uint index) const { return batchMaterials[index]; }
...
protected:
	typedef std::vector<std::string> MaterialIdList;
	...
	Array <Batch> batches;
	MaterialIdList batchMaterials;
	MaterialLibResId materialLibId;
